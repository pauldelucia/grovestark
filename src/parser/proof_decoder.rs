use crate::error::{Error, Result};
use crate::parser::proof_extractor::{decode_merk_proof, extract_merkle_path_from_ops};
use crate::types::MerkleNode;

/// Parse GroveDB proof from DET_PROOF_LOGS format
/// This format is bincode-encoded with Merk operations embedded deep in the structure
pub fn parse_det_proof_format(raw_proof: &[u8]) -> Result<Vec<MerkleNode>> {
    if raw_proof.len() < 100 {
        return Err(Error::Parser("Proof too short".into()));
    }

    // This is a bincode-encoded GroveDBProof
    // First byte is the variant (0x00 for V0)
    if raw_proof[0] != 0x00 {
        return Err(Error::Parser(format!(
            "Expected GroveDBProof V0 variant (0x00), got 0x{:02x}",
            raw_proof[0]
        )));
    }

    // The actual Merk operations are embedded in the bincode structure
    // Based on analysis, they typically start around position 109-140
    // We need to search for the characteristic pattern

    // Search for the first clear sequence of Merk operations
    let mut merk_start = None;
    for i in 100..raw_proof.len().min(200) {
        // Look for common operation patterns
        if i + 33 <= raw_proof.len() {
            let byte = raw_proof[i];
            if byte == 0x01 || byte == 0x02 || byte == 0x10 || byte == 0x11 {
                // Verify this looks like a valid operation sequence
                if byte == 0x01 && i + 33 < raw_proof.len() {
                    // Check if next op after the hash is valid
                    let next = raw_proof[i + 33];
                    if next == 0x01 || next == 0x02 || next == 0x10 || next == 0x11 {
                        merk_start = Some(i);
                        break;
                    }
                } else if byte == 0x02 && i + 1 < raw_proof.len() {
                    // Parent operation followed by something
                    let next = raw_proof[i + 1];
                    if next == 0x01 || next == 0x10 {
                        merk_start = Some(i);
                        break;
                    }
                }
            }
        }
    }

    let start = merk_start.unwrap_or(36);

    // Try to decode operations from the found start position
    match decode_merk_proof(&raw_proof[start..]) {
        Ok(ops) => {
            let path = extract_merkle_path_from_ops(&ops);

            // Convert to MerkleNodes
            Ok(path
                .into_iter()
                .map(|(is_left, hash)| MerkleNode { hash, is_left })
                .collect())
        }
        Err(_) => {
            // If direct decoding fails, try a more lenient approach
            // Look for sequences that look like 32-byte hashes
            let mut nodes = Vec::new();
            let mut i = start;

            while i + 32 <= raw_proof.len() {
                let byte = raw_proof[i];

                match byte {
                    0x01 => {
                        // Push operation with hash
                        if i + 33 <= raw_proof.len() {
                            let mut hash = [0u8; 32];
                            hash.copy_from_slice(&raw_proof[i + 1..i + 33]);

                            // Check if next byte is Parent (0x02)
                            let is_left = i + 33 < raw_proof.len() && raw_proof[i + 33] == 0x02;

                            nodes.push(MerkleNode { hash, is_left });
                            i += 33;
                            if is_left {
                                i += 1; // Skip the Parent op
                            }
                        } else {
                            i += 1;
                        }
                    }
                    0x02 => {
                        // Parent operation - already handled above
                        i += 1;
                    }
                    0x10 => {
                        // KVHash operation
                        if i + 33 <= raw_proof.len() {
                            let mut hash = [0u8; 32];
                            hash.copy_from_slice(&raw_proof[i + 1..i + 33]);
                            nodes.push(MerkleNode {
                                hash,
                                is_left: false,
                            });
                            i += 33;
                        } else {
                            i += 1;
                        }
                    }
                    _ => {
                        i += 1;
                    }
                }

                // Stop if we've found enough nodes for a reasonable path
                if nodes.len() > 30 {
                    break;
                }
            }

            if nodes.is_empty() {
                Err(Error::Parser("No Merkle nodes found in proof".into()))
            } else {
                Ok(nodes)
            }
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use hex;

    #[test]
    fn test_parse_det_proof() {
        let doc_proof_hex = "";

        let doc_proof = hex::decode(doc_proof_hex).unwrap();
        let nodes = parse_det_proof_format(&doc_proof).unwrap();

        assert!(!nodes.is_empty());
        println!("Extracted {} Merkle nodes", nodes.len());
    }
}
