use grovestark::parser::{parse_grovedb_proof_full, proof_extractor::*};
use hex;

#[test]
fn test_parse_det_proof_logs_data() {
    // Document proof from DET_PROOF_LOGS.md
    let doc_proof_hex = "";

    let doc_proof = hex::decode(doc_proof_hex).expect("Failed to decode hex");

    println!("=== Parsing Document Proof ===");
    println!("Proof size: {} bytes", doc_proof.len());

    // Extract state root
    let state_root = &doc_proof[0..32];
    println!("State root: {}", hex::encode(state_root));

    // Extract proof length
    let proof_len = u32::from_le_bytes(doc_proof[32..36].try_into().unwrap()) as usize;
    println!("Proof length field: {} bytes", proof_len);

    // Extract Merk proof data
    let merk_data = &doc_proof[36..36 + proof_len.min(doc_proof.len() - 36)];
    println!("Merk data size: {} bytes", merk_data.len());

    // Decode operations
    match decode_merk_proof(merk_data) {
        Ok(ops) => {
            println!("\n=== Decoded Operations ===");
            println!("Total operations: {}", ops.len());

            for (i, op) in ops.iter().enumerate().take(20) {
                match op {
                    MerkOp::Push(node) => println!("  {}: Push({:?})", i, node_type_name(node)),
                    MerkOp::PushInverted(node) => {
                        println!("  {}: PushInverted({:?})", i, node_type_name(node))
                    }
                    MerkOp::Parent => println!("  {}: Parent", i),
                    MerkOp::Child => println!("  {}: Child", i),
                    MerkOp::ParentInverted => println!("  {}: ParentInverted", i),
                    MerkOp::ChildInverted => println!("  {}: ChildInverted", i),
                }
            }

            // Extract Merkle path
            let path = extract_merkle_path_from_ops(&ops);
            println!("\n=== Extracted Merkle Path ===");
            println!("Path length: {} nodes", path.len());

            for (i, (is_left, hash)) in path.iter().enumerate() {
                println!(
                    "  Node {}: is_left={}, hash={}",
                    i,
                    is_left,
                    hex::encode(&hash[..8])
                );
            }

            // Try the full parsing function
            println!("\n=== Using parse_grovedb_proof_full ===");
            match parse_grovedb_proof_full(&doc_proof) {
                Ok((doc_nodes, key_nodes)) => {
                    println!("Document path: {} nodes", doc_nodes.len());
                    println!("Key path: {} nodes", key_nodes.len());

                    for (i, node) in doc_nodes.iter().enumerate().take(5) {
                        println!(
                            "  Doc node {}: is_left={}, hash={}",
                            i,
                            node.is_left,
                            hex::encode(&node.hash[..8])
                        );
                    }
                }
                Err(e) => {
                    println!("Failed to parse with parse_grovedb_proof_full: {:?}", e);
                }
            }
        }
        Err(e) => {
            println!("Failed to decode operations: {:?}", e);

            // Try to find where the Merk operations actually start
            println!("\n=== Searching for Merk operations ===");
            for i in 0..merk_data.len().min(100) {
                let byte = merk_data[i];
                if byte == 0x01 || byte == 0x02 || byte == 0x10 || byte == 0x11 {
                    println!("Found potential op 0x{:02x} at position {}", byte, i);

                    // Try decoding from this position
                    if let Ok(ops) = decode_merk_proof(&merk_data[i..]) {
                        println!(
                            "Successfully decoded {} ops starting at position {}",
                            ops.len(),
                            i
                        );
                        break;
                    }
                }
            }
        }
    }
}

fn node_type_name(node: &MerkNode) -> &'static str {
    match node {
        MerkNode::Hash(_) => "Hash",
        MerkNode::KVHash(_) => "KVHash",
        MerkNode::KV(_, _) => "KV",
        MerkNode::KVValueHash(_, _, _) => "KVValueHash",
        MerkNode::KVDigest(_, _) => "KVDigest",
        MerkNode::KVRefValueHash(_, _, _) => "KVRefValueHash",
        MerkNode::KVValueHashFeatureType(_, _, _, _) => "KVValueHashFeatureType",
    }
}

#[test]
fn test_parse_key_proof() {
    // Key proof from DET_PROOF_LOGS.md
    let key_proof_hex = "";

    let key_proof = hex::decode(key_proof_hex).expect("Failed to decode hex");

    println!("\n=== Parsing Key Proof ===");
    println!("Proof size: {} bytes", key_proof.len());

    // Parse using the full function
    match parse_grovedb_proof_full(&key_proof) {
        Ok((doc_nodes, key_nodes)) => {
            println!("Document path: {} nodes", doc_nodes.len());
            println!("Key path: {} nodes", key_nodes.len());

            let total_nodes = doc_nodes.len() + key_nodes.len();
            println!("Total Merkle nodes extracted: {}", total_nodes);
        }
        Err(e) => {
            println!("Failed to parse key proof: {:?}", e);
        }
    }
}
